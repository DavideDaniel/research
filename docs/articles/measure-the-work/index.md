...
Token billing itemized it. [GitHub's move of Copilot to usage-based billing](https://github.blog/news-insights/company-news/github-copilot-is-moving-to-usage-based-billing/) (April 27, 2026) is the clearest single marker: the largest developer-tools vendor replaced a flat seat with a meter. And meters get audited.
...
The enterprises that adopted agentic tools at scale were never on seat pricing for them. A flat seat cannot exhaust an annual budget ahead of schedule. Only a meter can. Those companies were on usage-based pricing a full budget cycle before the Copilot pivot brought it to everyone else. Their blown budgets were already circulating as cautionary tales when the meter reached the broader market. The panic arrived pre-loaded.

Those are real operational problems, and the [FinOps playbook](/articles/finops-for-agents/) covers the levers that contain them: caps, caching, gateways, routing. Contain is the honest verb. The savings are unbenchmarked until measured against your own workload, and the routing half increasingly ships inside the model vendors' own products rather than as something to build. But a second reaction is spreading alongside the operational one, and it is the one worth slowing down: the demand that token spend be traced, line by line, to business value. **"What value did these tokens create"** is becoming a standing question in budget reviews.
...
**Pick the measurement start line at today, and instrument forward from it.** The infrastructure is the same gateway layer the FinOps playbook already argued for, with one addition. Tag agent traffic with task identity, so token spend joins to the delivered change it produced: the pull request, the ticket, the shipped artifact.

The gateway already logs *which identity, which model, how many tokens*. Adding *for which piece of work* converts the input meter into the metric that matters: **cost per delivered change**. Alongside it sit cycle time per delivered change and rework rate, all trending from a known start date.
